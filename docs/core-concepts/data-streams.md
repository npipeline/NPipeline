...
In NPipeline, data flows between nodes through **Data Streams**. These pipes are the fundamental mechanism for transferring items from one processing stage to the next. At their core, data streams implement the `IDataStream` interface, with generic variations for typed data.

## `IDataStream` and `IDataStream<T>`

`IDataStream` is the non-generic interface representing a data channel. It's primarily used internally for untyped operations or when the data type is not relevant to the pipe's management.
...
**Note:** The `ToAsyncEnumerable()` method is an internal framework API and should not be called by external code. External APIs should use the typed `IDataStream<T>` interface directly, which implements `IAsyncEnumerable<T>` and is both type-safe and zero-overhead.

`IDataStream<T>` is the generic interface, where `T` represents the type of data item flowing through that specific pipe. This is the interface you will most commonly interact with when defining the connections between your nodes. It provides methods for asynchronously producing and consuming streams of data.

```csharp
public interface IDataStream<out T> : IDataStream, IAsyncEnumerable<T> { }
```

Conceptually, a data stream acts as a conduit:
...
* **Flow Control:** NPipeline's internal implementation of data streams manages the flow of data, preventing downstream nodes from being overwhelmed and upstream nodes from producing too quickly.
* **Strongly Typed:** `IDataStream<T>` ensures type safety throughout your pipeline, reducing runtime errors.
* **Stream Identification:** Each data stream has a `StreamName` property for identification and debugging purposes.
* **Type Information:** The `GetDataType()` method allows runtime discovery of the data type flowing through the pipe.

## Example Usage (Conceptual)

While you typically don't interact directly with `IDataStream<T>` instances when building a pipeline with `PipelineBuilder`, it's the underlying mechanism. When you connect nodes, NPipeline implicitly creates and manages these pipes.
...
{
    public async IAsyncEnumerable<int> TransformAsync(IAsyncEnumerable<string> input, CancellationToken cancellationToken = default)
    {
...
In this example, the `input` parameter is an `IAsyncEnumerable<string>`, which is backed by an `IDataStream<string>`. The `yield return number;` effectively pushes data into the output `IDataStream<int>` managed by NPipeline.

## Consuming a Data Stream

When you implement a node (typically a Sink or Transform), you receive data from upstream nodes via an `IDataStream<T>`. You can consume the data using a standard `await foreach` loop:
...
In a source node, you are responsible for creating the initial data stream. NPipeline provides helpers to easily create a pipe from an `IAsyncEnumerable<T>` or a simple `IEnumerable<T>`:
...
public class MySourceNode : SourceNode<MyData>
{
    public IDataStream<MyData> ExecuteAsync(PipelineContext context, CancellationToken cancellationToken)
    {
        ...
        // Create a data stream from an async stream
        return new DataStream<MyData>(Stream());
    }
}
```

By abstracting the data flow into `IDataStream<T>`, NPipeline allows you to build complex, high-performance streaming workflows with a simple and consistent programming model.
...
var pipe = source.OpenStream(context, cancellationToken);  // Returns immediately
```
...
2. **Type Safety:** Enables covariance on `IDataStream<T>` (not possible with `Task<T>` wrapper)
...
var pipe = source.OpenStream(...);      // Fast, synchronous
...
1. An `IDataStream<string>` between `MyStringSource` and `StringToIntTransform`.
2. An `IDataStream<int>` between `StringToIntTransform` and `MyIntSink`.
...
* **[Nodes](nodes/index.md)**: Learn about the different types of nodes (Source, Transform, Sink) that interact with data streams.
