...
namespace Sample_13_KeyedJoinNode;

/// <summary>
///     Entry point for the Keyed Join Node sample demonstrating KeyedJoinNode functionality with NPipeline.
///     This sample shows how to use KeyedJoinNode to join data streams based on common keys,
///     demonstrating different join strategies and data enrichment patterns.
/// </summary>
public sealed class Program
{
    public static async Task Main(string[] args)
    {
        Console.WriteLine("=== NPipeline Sample: Keyed Join Node ===");
        Console.WriteLine();
        ...
        try
        {
            ...
            Console.WriteLine(KeyedJoinPipeline.GetDescription());
            Console.WriteLine();
    ...
    private static async Task DemonstrateJoinTypes(IHost host)
    {
        ...
        foreach (var joinType in joinTypes)
        {
            ...
            var pipeline = new KeyedJoinPipeline(joinType);
            ...
            var parameters = new Dictionary<string, object> { ["JoinType"] = joinType };
            await host.Services.RunPipelineAsync<KeyedJoinPipeline>(parameters, CancellationToken.None);
