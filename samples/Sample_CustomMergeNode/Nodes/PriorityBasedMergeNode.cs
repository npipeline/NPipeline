...
public class PriorityBasedMergeNode : CustomMergeNode<MarketDataTick>, ITransformNode<MarketDataTick, MarketDataTick>
{
    ...
    public async ValueTask<MarketDataTick> TransformAsync(MarketDataTick input, PipelineContext context, CancellationToken cancellationToken) =>
        ...
        await ValueTask.FromResult<MarketDataTick>(input);
    ...
    public override Task<IDataStream<MarketDataTick>> MergeAsync(IEnumerable<IDataStream> pipes, CancellationToken cancellationToken = default)
    {
        ...
        var typedPipes = pipes.Cast<IDataStream<MarketDataTick>>().ToList();
