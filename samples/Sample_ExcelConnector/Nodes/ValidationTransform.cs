...
public partial class ValidationTransform(bool filterInvalidRecords = true) : TransformNode<Customer, Customer>
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (input.AccountBalance < 0)
            errors.Add($"Account balance cannot be negative: {input.AccountBalance}");

        // Validate discount percentage
        if (input.DiscountPercentage is < 0 or > 100)
            errors.Add($"Discount percentage must be between 0 and 100: {input.DiscountPercentage}");

        // Validate loyalty points
        if (input.LoyaltyPoints < 0)
            errors.Add($"Loyalty points cannot be negative: {input.LoyaltyPoints}");
