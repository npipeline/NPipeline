...
public class ConsoleSink<T> : SinkNode<T>
{
    ...
    private static string FormatOutput(T? item)
    {
        return item switch
        {
            OrderCustomerJoin join => $"Order-Customer Join - Order {join.Order.OrderId} ↔ Customer {join.Customer.Name} (Tier: {join.CustomerTier})",
            EnrichedOrder enriched =>
                $"Enriched Order {enriched.Order.OrderId} - {enriched.CustomerName} bought {enriched.ProductName} ({enriched.ProductCategory}) - {enriched.Order.TotalPrice:C}",
            SalesByCustomerTier tierSales =>
                $"Tier {tierSales.CustomerTier} Sales: {tierSales.TotalOrders} orders, {tierSales.TotalRevenue:C} revenue, {tierSales.AverageOrderValue:C} avg",
            SalesByCategory categorySales =>
                $"Category {categorySales.Category}: {categorySales.TotalOrders} orders, {categorySales.TotalQuantity} units, {categorySales.TotalRevenue:C} revenue, Top: {categorySales.TopProduct}",
            Customer customer => $"Customer {customer.CustomerId}: {customer.Name} ({customer.Email}) - Tier: {customer.CustomerTier}",
            Order order => $"Order {order.OrderId}: Customer {order.CustomerId}, Product {order.ProductCode}, Qty {order.Quantity}, Total {order.TotalPrice:C}",
            Product product => $"Product {product.ProductCode}: {product.ProductName} ({product.Category}) - {product.Price:C}",
            null => $"[{typeof(T).Name}]",
