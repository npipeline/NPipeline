...
```bash
dotnet test --project Sample_TypeConversionNode.csproj
```
...
```bash
dotnet run --project Sample_TypeConversionNode.csproj
```

### With Custom Parameters

```bash
dotnet run --project Sample_TypeConversionNode.csproj -- --record-count 20 --enable-error-handling
```
