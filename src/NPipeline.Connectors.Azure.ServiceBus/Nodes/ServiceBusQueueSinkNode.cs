...
/// <summary>
///     Sink node that publishes messages to an Azure Service Bus <em>queue</em>.
///     Supports batching (up to 100 messages) and automatic source-message acknowledgment.
/// </summary>
/// <typeparam name="T">
///     The type of data to publish. When <typeparamref name="T" /> implements
///     <see cref="IAcknowledgableMessage" />, the source message is acknowledged after a successful send
///     (subject to <see cref="ServiceBusConfiguration.AcknowledgmentStrategy" />).
/// </typeparam>
/// <remarks>
///     <para>
///         A dedicated <see cref="ServiceBusSender" /> is created per node. For shared senders across
///         multiple nodes publishing to the same queue, use the DI registration with
///         <see cref="DependencyInjection.ServiceBusServiceCollectionExtensions" /> and inject
...
public sealed class ServiceBusQueueSinkNode<T> : SinkNode<T>
{
    ...
    /// <summary>
    ///     Creates a <see cref="ServiceBusQueueSinkNode{T}" /> from the supplied configuration.
    ///     A dedicated <see cref="ServiceBusClient" /> is created and owned by this node.
    /// </summary>
    public ServiceBusQueueSinkNode(ServiceBusConfiguration configuration, ILogger? logger = null)
    {
        ...
        {
            throw new ArgumentException("QueueName must be specified for a queue sink node.",
        ...
        _sender = _ownedClient.CreateSender(_configuration.QueueName);
        _ownsClient = true;
    }

    /// <summary>
    ///     Creates a <see cref="ServiceBusQueueSinkNode{T}" /> with an injected sender (testing or pool).
    /// </summary>
