...
namespace NPipeline.Connectors.PostgreSQL.Analyzers;

/// <summary>
///     Analyzer that detects PostgreSQL source nodes with checkpointing enabled but missing ORDER BY clause.
...
public sealed class PostgresCheckpointOrderingAnalyzer : DiagnosticAnalyzer
{
    ...
    private static readonly DiagnosticDescriptor PostgresCheckpointOrderingRule = new(
    ...
    private static void AnalyzeObjectCreation(SyntaxNodeAnalysisContext context)
    {
        ...
        // Check if this is a PostgresSourceNode<T> instantiation
        var typeSymbol = context.SemanticModel.GetTypeInfo(objectCreation).Type;

        if (typeSymbol == null)
            return;

        // Check if the type is PostgresSourceNode<T>
        if (!IsPostgresSourceNode(typeSymbol))
            return;
    ...
    /// <summary>
    ///     Determines if a type symbol represents PostgresSourceNode&lt;T&gt;.
    /// </summary>
    private static bool IsPostgresSourceNode(ITypeSymbol typeSymbol)
    {
        // Check the type name
        if (typeSymbol.Name != "PostgresSourceNode")
            return false;
        ...
        // Check if it's from the PostgreSQL connectors namespace
        return containingNamespace.Contains("PostgreSQL") &&
               containingNamespace.Contains("Nodes");
    }

    /// <summary>
    ///     Extracts the SQL query string from a PostgresSourceNode constructor.
    /// </summary>
    ...
    /// <summary>
    ///     Extracts the checkpoint strategy from a PostgresSourceNode constructor.
    /// </summary>
        ...
        SemanticModel semanticModel)
    {
        ...
        // PostgresSourceNode constructor signature:
        ...
        // If no checkpoint strategy argument, look for PostgresConfiguration in 2nd argument
        if (arguments != null && arguments.Value.Count >= 2)
        {
            var secondArg = arguments.Value[1];
        ...
        SemanticModel semanticModel)
    {
        ...
        // If it's an object creation (new PostgresConfiguration()), try to analyze it
        if (expression is ObjectCreationExpressionSyntax configCreation)
        {
            ...
            return "None"; // Default assumption - matches PostgresConfiguration default
        }
