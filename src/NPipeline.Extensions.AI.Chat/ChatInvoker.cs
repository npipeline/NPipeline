...
internal static class ChatInvoker
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            throw new ChatTransformException("LLM returned a null or empty response.",
        ...
        try
        {
            ...
            if (result is null)
            {
                throw new ChatTransformException("LLM returned a JSON null value. Expected a typed object.",
        ...
        catch (Exception ex) when (ex is JsonException or NotSupportedException)
        {
            ...
            throw new ChatTransformException("Failed to deserialize LLM response.", ex)
            {
                OriginalItem = item,
                PromptSent = userMessage,
                RawResponse = rawResponse,
        ...
        Action<ChatOptions>? configureOptions)
    {
        ...
        if (configureOptions is not null)
        {
            ...
            catch (Exception ex)
            {
                ...
                throw new ChatTransformException("ConfigureOptions delegate failed.", ex)
                {
                    OriginalItem = item,
                    PromptSent = userMessage,
                };
    ...
    private static string BuildUserMessage<T>(T item, Func<T, string> template, string templateName)
    {
        ...
        catch (Exception ex)
        {
            ...
            throw new ChatTransformException($"{templateName} delegate failed.", ex)
            {
                OriginalItem = item,
            };
        }
    ...
    private static ChatTransformException WrapException(object? item, string userMessage, Exception inner, string? modelId) =>
        new("AI transform failed.", inner)
        {
            OriginalItem = item,
            PromptSent = userMessage,
            ModelUsed = modelId,
