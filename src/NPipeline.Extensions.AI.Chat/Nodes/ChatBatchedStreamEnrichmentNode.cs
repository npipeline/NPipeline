...
public sealed class ChatBatchedStreamEnrichmentNode<TIn, TField> : IStreamTransformNode<TIn, TIn>, IExecutionStrategyProvider
{
    ...
    public ChatBatchedStreamEnrichmentNode(IChatClient chatClient)
    {
        _chatClient = chatClient ?? throw new ArgumentNullException(nameof(chatClient));
    }

    /// <summary>Gets or sets the stream batched enrich options including batch size and result mapper.</summary>
    public ChatBatchedStreamEnrichmentOptions<TIn, TField> Options { get; set; } = new();
    ...
    private async Task<IReadOnlyCollection<TIn>> ProcessBatchAsync(
        ...
        ChatBatchedStreamEnrichmentOptions<TIn, TField> options,
