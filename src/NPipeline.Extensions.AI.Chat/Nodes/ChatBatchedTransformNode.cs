...
public sealed class ChatBatchedTransformNode<TIn, TOut> : TransformNode<IReadOnlyCollection<TIn>, IReadOnlyCollection<TOut>>
{
    ...
    public ChatBatchedTransformNode(IChatClient chatClient)
    {
        _chatClient = chatClient ?? throw new ArgumentNullException(nameof(chatClient));
    }

    /// <summary>Gets or sets the batched transform options.</summary>
    public ChatBatchedTransformOptions<TIn, TOut> Options { get; set; } = new();
        ...
        CancellationToken cancellationToken)
    {
        ...
        return await ChatInvoker.InvokeBatchedTransformAsync<TIn, TOut>(
            _chatClient,
            item,
