...
public sealed class ChatEnrichmentNode<TIn, TField> : TransformNode<TIn, TIn>
{
    ...
    public override async ValueTask<TIn> TransformAsync(TIn item, PipelineContext context, CancellationToken cancellationToken)
    {
        ...
        var aiResult = await ChatInvoker.InvokeEnrichAsync<TIn, TField>(
            _chatClient,
            item,
