...
///     </para>
///     <para>
///         On construction, it records the node start event (if timing is enabled).
///         On disposal, it records all configured metrics including timing, item counts,
///         and performance metrics.
///     </para>
/// </remarks>
public struct AutoObservabilityScope : IDisposable
{
        ...
        ObservabilityOptions options)
    {
        ...
        _initialMemoryBytes = options.RecordMemoryUsage ? GC.GetTotalMemory(false) : 0;
        _threadId = options.RecordThreadInfo ? Environment.CurrentManagedThreadId : null;
        _itemsProcessed = 0;
        _itemsEmitted = 0;
        _success = true;
        _exception = null;
        ...
        // Record node start if timing is enabled
        if (options.RecordTiming)
        {
            collector.RecordNodeStart(
                ...
                _threadId,
                options.RecordMemoryUsage ? _initialMemoryBytes / 1024 / 1024 : null);
        }
    }
    ...
    /// <summary>
    ///     Disposes the scope and records all configured metrics.
    /// </summary>
    public void Dispose()
    {
        if (_disposed)
        ...
        // Record node end if timing is enabled
