        ...
        CancellationToken cancellationToken)
    {
        ...
        if (boundedCapacity is null)
        {
            logger.Log(LogLevel.Warning,
                "Parallel node {NodeId} has unbounded input queue (MaxQueueLength null). This may increase memory under sustained load.", nodeId);
        }

        if (parallelOptions?.OutputBufferCapacity is null)
        {
            logger.Log(LogLevel.Warning,
                "Parallel node {NodeId} has unbounded output buffer (OutputBufferCapacity null). Downstream slowness can accumulate results in memory.",
                nodeId);
        }
