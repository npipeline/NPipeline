...
public static class ParallelMetricsExtensions
{
    ...
    {
        if (context.NodeEnvironment.NodeExecutionScopeRegistry.TryGetRuntimeAnnotation(Key(nodeId), out var value) && value is ParallelExecutionMetrics m)
        {
            metrics = m;
            return true;
        }
    ...
    {
        if (context.TryGetParallelMetrics(nodeId, out var m))
            return m;
