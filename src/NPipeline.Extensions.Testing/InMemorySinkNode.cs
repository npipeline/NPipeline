...
/// <summary>
///     A sink node that collects all received items into an in-memory collection for inspection.
///     Thread-safe for use with parallel execution strategies.
/// </summary>
...
public class InMemorySinkNode<T> : SinkNode<T>
{
    ...
    /// <summary>
    ///     Gets a thread-safe snapshot of the items that have been received by the sink.
    /// </summary>
    ...
    public override async Task ExecuteAsync(IDataPipe<T> input, PipelineContext context, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            _completionSource.TrySetResult(_items.ToList());
        }
