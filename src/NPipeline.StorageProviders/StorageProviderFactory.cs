...
public static class StorageProviderFactory
{
        ...
        CreateResolverWithErrors(StorageResolverOptions? options = null)
    {
        ...
        if (options.IncludeFileSystem)
            TryRegisterFileSystemProvider(resolver, errors);
    ...
    private static void TryRegisterFileSystemProvider(
        StorageResolver resolver,
        Dictionary<string, IReadOnlyList<string>>? errors)
    {
        var type = Type.GetType(FileSystemProviderTypeName, false, false);
        ...
        {
            if (errors is not null)
                errors["FileSystem"] = [$"Type '{type.FullName}' does not have a public parameterless constructor."];
        ...
        try
        {
            if (Activator.CreateInstance(type) is IStorageProvider provider)
                resolver.RegisterProvider(provider);
            else if (errors is not null)
                errors["FileSystem"] = [$"Failed to construct '{type.FullName}'."];
        }
        catch (Exception ex)
        {
            if (errors is not null)
                errors["FileSystem"] = [$"Failed to construct '{type.FullName}': {ex.Message}"];
        }
    }
