...
internal sealed class CapAwareMaterializingStrategy<TIn, TOut> : ILineageMappingStrategy<TIn, TOut>
{
        ...
        [EnumeratorCancellation] CancellationToken ct)
    {
        ...
        if (!overflow)
        {
            ...
            foreach (var packet in LineageMappingHelpers.MapMaterialized(inBuf, outBuf, nodeId, cardinality, options, lineageMapperType,
                         mapperInstance))
            {
                yield return packet;
            }
        ...
        await foreach (var packet in LineageMappingHelpers.PositionalStreamingMap(InputAll(ct), OutputAll(ct), nodeId, cardinality, options, ct))
        {
            yield return packet;
        }
