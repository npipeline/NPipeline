...
public sealed class LineageService : ILineageService
{
    ...
    private static List<object?> MaterializePipeData(IDataPipe pipe)
    {
        ...
        // Note: Using GetAwaiter().GetResult() here is acceptable because:
        // 1. The task is already running on a thread pool thread via Task.Run
        // 2. All async operations inside use .ConfigureAwait(false) to avoid capturing sync context
        // 3. This method is synchronous and cannot be made async without breaking the API
