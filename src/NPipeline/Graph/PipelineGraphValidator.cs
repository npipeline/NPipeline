...
public static class PipelineGraphValidator
{
    ...
    private sealed class MultiInboundNonJoinRule : IGraphRule
    {
        public string Name => "MultiInboundNonJoin";
        public bool StopOnError => false;
        ...
        public IEnumerable<ValidationIssue> Evaluate(GraphValidationContext context)
        {
            ...
            {
                if (node.Kind is NodeKind.Transform or NodeKind.Sink && incoming.TryGetValue(node.Id, out var list) && list.Count > 1 && !node.IsJoin)
                {
                    yield return new ValidationIssue(
                        ValidationSeverity.Error,
                        $"Node '{node.Id}' ('{node.Name}', {node.NodeType.Name}) has multiple inbound edges but is not a join node.",
                        "Structure");
                }
            }
        }
