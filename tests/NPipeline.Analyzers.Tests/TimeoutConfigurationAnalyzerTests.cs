...
";
    ...
    [Fact]
    public async Task Analyzer_ShouldReportDiagnostic_WhenRetryTimeoutIsExcessive()
    {
        var test = @"
using NPipeline;
...
";
        ...
        var expectedDiagnostic = VerifyCS.Diagnostic(TimeoutConfigurationAnalyzer.DiagnosticId)
            ...
            .WithArguments(
                "timeout: TimeSpan.FromHours(1)",
                "retry",
                "Retry timeouts should not exceed 30 minutes to prevent excessive resource consumption.");
...
";
    ...
    [Fact]
    public async Task Analyzer_ShouldProvideCodeFix_ForRetryTimeout()
    {
        var test = @"
using NPipeline;
