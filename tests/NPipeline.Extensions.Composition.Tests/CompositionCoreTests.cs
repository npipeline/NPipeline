...
public class CompositionCoreTests
{
    ...
    public async Task AddComposite_WithServiceProvider_ShouldResolveChildDefinition()
    {
        ...
        var definition = new ParentWithDiCompositePipeline(serviceProvider, false);
    ...
    public async Task AddComposite_WithServiceProviderMissingRegistration_ShouldThrowByDefault()
    {
        ...
        var runner = PipelineRunner.Create();
        var context = new PipelineContext();
        var definition = new ParentWithDiCompositePipeline(new DictionaryServiceProvider(), false);

        // Act
        var act = () => runner.RunAsync(definition, context);
    ...
    {
        public override ValueTask<int> TransformAsync(int input, PipelineContext context, CancellationToken cancellationToken)
            => ValueTask.FromResult(input * factor);
    }
