...
public class NumericValidationNodeTests
{
    ...
    public async Task ValidationError_Contains_PropertyPath()
    {
        ...
        var exception = await Assert.ThrowsAsync<ValidationException>(() => node.TransformAsync(item, PipelineContext.CreateDefault(), CancellationToken.None));
    ...
    public async Task IsPositive_WithPositiveInt_Passes()
    {
        ...
        var item = new TestObject { IntValue = 5 };
        var result = await node.TransformAsync(item, PipelineContext.CreateDefault(), CancellationToken.None);
