...
public sealed class WatermarksTests
{
    ...
    #region WatermarkTimeHelper Tests

    [Fact]
    public void WatermarkTimeHelper_SafeSubtract_WithValidTimestamps_SubtractsCorrectly()
    {
        // Arrange
        DateTimeOffset timestamp = new(2024, 1, 15, 10, 30, 45, TimeSpan.Zero);
        ...
        // Act
        var result = WatermarkTimeHelper.SafeSubtract(timestamp, delta);
    ...
    [Fact]
    public void WatermarkTimeHelper_SafeSubtract_WithMinValueTimestamp_ReturnsMinValue()
    {
        // Arrange
        var delta = TimeSpan.FromSeconds(10);

        // Act
        var result = WatermarkTimeHelper.SafeSubtract(DateTimeOffset.MinValue, delta);

        // Assert
        _ = result.Should().Be(DateTimeOffset.MinValue);
    }

    [Fact]
    public void WatermarkTimeHelper_SafeSubtract_WithVeryLargeDelta_ReturnsMinValue()
    {
        // Arrange - create a timestamp that's close to now, and subtract more than distance to MinValue
        var timestamp = DateTimeOffset.MinValue.AddDays(10); // Only 10 days from MinValue
        var massiveDelta = TimeSpan.FromDays(365 * 100); // 100 years, way more than 10 days

        // Act
        var result = WatermarkTimeHelper.SafeSubtract(timestamp, massiveDelta);

        // Assert - should prevent underflow by returning MinValue since delta exceeds distance to min
        _ = (result == DateTimeOffset.MinValue).Should().BeTrue();
    }

    [Fact]
    public void WatermarkTimeHelper_SafeSubtract_WithZeroDelta_ReturnsOriginalTimestamp()
    {
        // Arrange
        DateTimeOffset timestamp = new(2024, 1, 15, 10, 30, 45, TimeSpan.Zero);
        ...
        // Assert
        _ = result.Should().Be(timestamp);
    }

    [Fact]
    public void WatermarkTimeHelper_SafeSubtract_WithNegativeDelta_AddsToTimestamp()
    {
        // Arrange
        DateTimeOffset timestamp = new(2024, 1, 15, 10, 30, 45, TimeSpan.Zero);
        ...
        // Act
        var result = WatermarkTimeHelper.SafeSubtract(timestamp, negativeDelta);
