...
public sealed class EqualJitterStrategyTests
{
    ...
    public void ApplyJitter_WithOddBaseDelay_ShouldCalculateCorrectly()
    {
        ...
        _ = result.Should().BeLessThan(TimeSpan.FromMilliseconds(1001));
    }
    ...
    public void ApplyJitter_WithMultipleCalls_ShouldProduceDifferentResults()
    {
        ...
        {
            _ = result.Should().BeGreaterThanOrEqualTo(TimeSpan.FromMilliseconds(500));
            _ = result.Should().BeLessThan(TimeSpan.FromMilliseconds(1000));
        }
    }
    ...
    public void ApplyJitter_StatisticalDistribution_ShouldBeInCorrectRange()
    {
        ...
        // Average should be roughly 3/4 of the range (with some tolerance)
        var average = results.Average();
        average.Should().BeGreaterThan(700); // Lower bound check
        average.Should().BeLessThan(800); // Upper bound check
    }
