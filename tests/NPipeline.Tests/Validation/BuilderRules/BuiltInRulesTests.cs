...
public sealed class BuiltInRulesTests
{
    ...
    {
        var b = new PipelineBuilder().WithExtendedValidation();
    ...
    [Fact]
    public void MultiInboundNonJoinRule_Fires()
    {
        var b = new PipelineBuilder().WithExtendedValidation();
        ...
        var t = b.AddTransform<IntToString, int, string>("t");
        b.Connect(s1, t);
        b.Connect(s2, t); // second inbound -> error

        // Add a sink to avoid MissingSinkRule dominating the result so multi inbound rule is asserted.
        var sink = b.AddSink<StringSink, string>("sink");
        b.Connect(t, sink);
        var ok = b.TryBuild(out _, out var result);
        ok.Should().BeFalse();
        result.Issues.Should().Contain(i => i.Message.Contains("multiple inbound"));
    }
